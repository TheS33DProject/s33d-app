...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({
  ...
  const isS33dBalanceInsufficient = s33dBalance.lt(pricePerTicket)
  const maxBuyTickets = Math.min(s33dBalance.div(pricePerTicket).toNumber(), remainingTickets)
        ...
        <Flex flexDirection="column">
          <Box px="16px">
            ...
            <Flex mb="8px" justifyContent="space-between">
              ...
              <Text font-size="14px" color={isS33dBalanceInsufficient ? 'failure' : 'text'}>
                {formatBigNumber(s33dBalance, 3)} CAKE
              </Text>
            </Flex>
            <Flex
              mb="8px"
          ...
          <Box px="16px">
            <Button
              disabled={isS33dBalanceInsufficient}
              onClick={() => buyTicketCallBack({ ticketsNumber })}
              width="100%"
            >
              {isS33dBalanceInsufficient ? t('Insufficient Balance') : t('Confirm')}
            </Button>
          </Box>
        </Flex>
      </ModalBody>
