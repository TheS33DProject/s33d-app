...
export const fetchFarmUserAllowances = async (account: string, farmsToFetch: SerializedFarmConfig[]) => {
  const grandGardenerAddress = getGrandGardenerAddress()

  const calls = farmsToFetch.map((farm) => {
    const lpContractAddress = getAddress(farm.lpAddresses)
    return { address: lpContractAddress, name: 'allowance', params: [account, grandGardenerAddress] }
  })
...
export const fetchFarmUserStakedBalances = async (account: string, farmsToFetch: SerializedFarmConfig[]) => {
  ...
  const calls = farmsToFetch.map((farm) => {
    return {
      address: grandGardenerAddress,
      name: 'userInfo',
      params: [farm.pid, account],
    }
  })

  const rawStakedBalances = await multicall(grandGardenerABI, calls)
...
export const fetchFarmUserEarnings = async (account: string, farmsToFetch: SerializedFarmConfig[]) => {
  ...
  const rawEarnings = await multicall(grandGardenerABI, calls)
